...
The Python SDK for the MonsterHunterWorld API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Ailment(None).list(None, None)
if err:
    raise Exception(err)
...
```python
result, err = client.Ailment(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.MonsterHunterWorld(None).load(
    {"id": "test01"}, None
)
