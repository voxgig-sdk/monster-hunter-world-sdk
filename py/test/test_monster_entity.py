...
class TestMonsterEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Monster(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Monster(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _monster_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "monster." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MONSTER_HUNTER_WORLD_TEST_MONSTER_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        monster_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.monster")))
        monster_ref01_data = None
        if len(monster_ref01_data_raw) > 0:
            monster_ref01_data = helpers.to_map(monster_ref01_data_raw[0][1])

        # LIST
        monster_ref01_ent = client.Monster(None)
        monster_ref01_match = {}

        monster_ref01_list_result = monster_ref01_ent.list(monster_ref01_match, None)
...
def _monster_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/monster/MonsterTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("MONSTER_HUNTER_WORLD_TEST_MONSTER_ENTID"))
