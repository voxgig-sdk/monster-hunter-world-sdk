# MonsterHunterWorld SDK test runner
...
class MonsterHunterWorldTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MonsterHunterWorldTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MonsterHunterWorldTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MonsterHunterWorldTestRunner.getenv("MONSTER_HUNTER_WORLD_TEST_LIVE")
        override = MonsterHunterWorldTestRunner.getenv("MONSTER_HUNTER_WORLD_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MonsterHunterWorldTestRunner.getenv(key)
        ...
        explain = MonsterHunterWorldTestRunner.getenv("MONSTER_HUNTER_WORLD_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if MonsterHunterWorldTestRunner._test_control is not None:
            return MonsterHunterWorldTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MonsterHunterWorldTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MonsterHunterWorldTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MonsterHunterWorldTestRunner.load_test_control()
...
def load_env_local():
    MonsterHunterWorldTestRunner.load_env_local()


def env_override(m):
    return MonsterHunterWorldTestRunner.env_override(m)


def entity_data(v):
    return MonsterHunterWorldTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MonsterHunterWorldTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MonsterHunterWorldTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MonsterHunterWorldTestRunner.load_test_control()


def live_delay_ms():
    return MonsterHunterWorldTestRunner.live_delay_ms()
