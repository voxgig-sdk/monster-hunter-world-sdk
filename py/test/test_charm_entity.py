    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MONSTER_HUNTER_WORLD_TEST_CHARM_ENTID JSON to run live")
        client = setup["client"]
        ...
        charm_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(charm_ref01_data_dt0_loaded))
...
def _charm_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "MONSTER_HUNTER_WORLD_TEST_CHARM_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("MONSTER_HUNTER_WORLD_TEST_CHARM_ENTID"))
