...
class TestWeaponEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Weapon(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Weapon(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MONSTER_HUNTER_WORLD_TEST_WEAPON_ENTID JSON to run live")
        client = setup["client"]
...
def _weapon_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "MONSTER_HUNTER_WORLD_TEST_WEAPON_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("MONSTER_HUNTER_WORLD_TEST_WEAPON_ENTID"))
