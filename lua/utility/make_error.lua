...
local function make_error_util(ctx, err)
  ...
  local msg = "MonsterHunterWorldSDK: " .. opname .. ": " .. errmsg
  ...
  local sdk_err = MonsterHunterWorldError.new("", msg, ctx)
  sdk_err.result = ctx.utility.clean(ctx, result)
  sdk_err.spec = ctx.utility.clean(ctx, spec)

  if type(err) == "table" and getmetatable(err) == MonsterHunterWorldError then
