...
describe("CharmDirect", function()
  it("should direct-list-charm", function()
    local setup = charm_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-charm", setup.live and "live" or "unit")
  ...
  it("should direct-load-charm", function()
    local setup = charm_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-charm", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function charm_direct_setup(mockres)
