...
describe("MotionValueDirect", function()
  it("should direct-list-motion_value", function()
    local setup = motion_value_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-motion_value", setup.live and "live" or "unit")
  ...
  it("should direct-load-motion_value", function()
    local setup = motion_value_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-motion_value", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function motion_value_direct_setup(mockres)
