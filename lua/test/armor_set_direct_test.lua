...
describe("ArmorSetDirect", function()
  it("should direct-list-armor_set", function()
    local setup = armor_set_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-armor_set", setup.live and "live" or "unit")
  ...
  it("should direct-load-armor_set", function()
    local setup = armor_set_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-armor_set", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function armor_set_direct_setup(mockres)
