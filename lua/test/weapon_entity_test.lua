...
describe("WeaponEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Weapon(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Weapon(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set MONSTERHUNTERWORLD_TEST_WEAPON_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    local weapon_ref01_data_dt0_load_result = helpers.to_map(weapon_ref01_data_dt0_loaded)
...
function weapon_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("MONSTERHUNTERWORLD_TEST_WEAPON_ENTID")
