...
describe("WeaponDirect", function()
  it("should direct-list-weapon", function()
    local setup = weapon_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-weapon", setup.live and "live" or "unit")
  ...
  it("should direct-load-weapon", function()
    local setup = weapon_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-weapon", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function weapon_direct_setup(mockres)
