...
describe("MonsterDirect", function()
  it("should direct-list-monster", function()
    local setup = monster_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-monster", setup.live and "live" or "unit")
  ...
  it("should direct-load-monster", function()
    local setup = monster_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-monster", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function monster_direct_setup(mockres)
