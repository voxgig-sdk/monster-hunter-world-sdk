...
describe("MonsterEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Monster(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Monster(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Monster(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = monster_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"list", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "monster." .. _op, _live and "live" or "unit")
    ...
    local monster_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.monster")))
    local monster_ref01_data = nil
    if #monster_ref01_data_raw > 0 then
      monster_ref01_data = helpers.to_map(monster_ref01_data_raw[1][2])
    end

    -- LIST
    local monster_ref01_ent = client:Monster(nil)
    local monster_ref01_match = {}

    local monster_ref01_list_result, err = monster_ref01_ent:list(monster_ref01_match, nil)
    assert.is_nil(err)
...
function monster_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/monster/MonsterTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read monster test data: " .. entity_data_file)
  end
