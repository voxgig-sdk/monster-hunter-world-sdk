...
function WeaponEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WeaponEntity)
  self._name = "weapon"
  self._client = client
...
function WeaponEntity:get_name()
  return self._name
end


function WeaponEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WeaponEntity.new(self._client, opts)
end
...
function WeaponEntity:mark_deleted()
  self._deleted = true
end


function WeaponEntity:deleted()
  return true == self._deleted
end


function WeaponEntity:data_set(args)
  if args ~= nil then
...
function WeaponEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WeaponEntity:match_set(args)
  if args ~= nil then
...
function WeaponEntity:match_get()
...
function WeaponEntity:stream(action, args, callopts)
...
---@param reqmatch WeaponLoadMatch
---@param ctrl? table
---@return Weapon
---@return string? err
function WeaponEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch WeaponListMatch
---@param ctrl? table
---@return Weapon[]
---@return string? err
function WeaponEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function WeaponEntity:_run_op(ctx, post_done)
  local utility = self._utility
