...
local DecorationEntity = {}
DecorationEntity.__index = DecorationEntity


function DecorationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DecorationEntity)
  self._name = "decoration"
  self._client = client
...
function DecorationEntity:get_name()
  return self._name
end


function DecorationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DecorationEntity.new(self._client, opts)
end
...
function DecorationEntity:mark_deleted()
  self._deleted = true
end


function DecorationEntity:deleted()
  return true == self._deleted
end


function DecorationEntity:data_set(args)
  if args ~= nil then
...
function DecorationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DecorationEntity:match_set(args)
  if args ~= nil then
...
function DecorationEntity:match_get()
...
function DecorationEntity:stream(action, args, callopts)
...
function DecorationEntity:_run_op(ctx, post_done)
  local utility = self._utility
