...
local ArmorSetEntity = {}
ArmorSetEntity.__index = ArmorSetEntity


function ArmorSetEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ArmorSetEntity)
  self._name = "armor_set"
  self._client = client
...
function ArmorSetEntity:get_name()
  return self._name
end


function ArmorSetEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ArmorSetEntity.new(self._client, opts)
end
...
function ArmorSetEntity:mark_deleted()
  self._deleted = true
end


function ArmorSetEntity:deleted()
  return true == self._deleted
end


function ArmorSetEntity:data_set(args)
  if args ~= nil then
...
function ArmorSetEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ArmorSetEntity:match_set(args)
  if args ~= nil then
...
function ArmorSetEntity:match_get()
...
function ArmorSetEntity:stream(action, args, callopts)
...
function ArmorSetEntity:_run_op(ctx, post_done)
  local utility = self._utility
