-- MonsterHunterWorld SDK Armor entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ArmorEntity = {}
ArmorEntity.__index = ArmorEntity


function ArmorEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ArmorEntity)
  self._name = "armor"
  self._client = client
...
function ArmorEntity:get_name()
  return self._name
end


function ArmorEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ArmorEntity.new(self._client, opts)
end
...
function ArmorEntity:mark_deleted()
  self._deleted = true
end


function ArmorEntity:deleted()
  return true == self._deleted
end


function ArmorEntity:data_set(args)
  if args ~= nil then
...
function ArmorEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ArmorEntity:match_set(args)
  if args ~= nil then
...
function ArmorEntity:match_get()
...
function ArmorEntity:stream(action, args, callopts)
...
---@param reqmatch ArmorLoadMatch
---@param ctrl? table
---@return Armor
---@return string? err
function ArmorEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch ArmorListMatch
---@param ctrl? table
---@return Armor[]
---@return string? err
function ArmorEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function ArmorEntity:_run_op(ctx, post_done)
  local utility = self._utility
