...
local CharmEntity = {}
CharmEntity.__index = CharmEntity


function CharmEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CharmEntity)
  self._name = "charm"
  self._client = client
...
function CharmEntity:get_name()
  return self._name
end


function CharmEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CharmEntity.new(self._client, opts)
end
...
function CharmEntity:mark_deleted()
  self._deleted = true
end


function CharmEntity:deleted()
  return true == self._deleted
end


function CharmEntity:data_set(args)
  if args ~= nil then
...
function CharmEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CharmEntity:match_set(args)
  if args ~= nil then
...
function CharmEntity:match_get()
...
function CharmEntity:stream(action, args, callopts)
...
---@param reqmatch CharmLoadMatch
---@param ctrl? table
---@return Charm
---@return string? err
function CharmEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch CharmListMatch
---@param ctrl? table
---@return Charm[]
---@return string? err
function CharmEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function CharmEntity:_run_op(ctx, post_done)
  local utility = self._utility
