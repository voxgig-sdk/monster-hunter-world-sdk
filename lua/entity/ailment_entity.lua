...
local AilmentEntity = {}
AilmentEntity.__index = AilmentEntity


function AilmentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AilmentEntity)
  self._name = "ailment"
  self._client = client
...
function AilmentEntity:get_name()
  return self._name
end


function AilmentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AilmentEntity.new(self._client, opts)
end
...
function AilmentEntity:mark_deleted()
  self._deleted = true
end


function AilmentEntity:deleted()
  return true == self._deleted
end


function AilmentEntity:data_set(args)
  if args ~= nil then
...
function AilmentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AilmentEntity:match_set(args)
  if args ~= nil then
...
function AilmentEntity:match_get()
...
function AilmentEntity:stream(action, args, callopts)
...
function AilmentEntity:_run_op(ctx, post_done)
  local utility = self._utility
