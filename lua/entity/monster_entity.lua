...
function MonsterEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, MonsterEntity)
  self._name = "monster"
  self._client = client
...
function MonsterEntity:get_name()
  return self._name
end


function MonsterEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return MonsterEntity.new(self._client, opts)
end
...
function MonsterEntity:mark_deleted()
  self._deleted = true
end


function MonsterEntity:deleted()
  return true == self._deleted
end


function MonsterEntity:data_set(args)
  if args ~= nil then
...
function MonsterEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function MonsterEntity:match_set(args)
  if args ~= nil then
...
function MonsterEntity:match_get()
...
function MonsterEntity:stream(action, args, callopts)
...
function MonsterEntity:_run_op(ctx, post_done)
  local utility = self._utility
