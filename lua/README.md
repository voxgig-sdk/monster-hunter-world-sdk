...
The Lua SDK for the MonsterHunterWorld API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Ailment(nil):list(nil, nil)
...
```lua
local result, err = client:Ailment(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:MonsterHunterWorld(nil):load(
  { id = "test01" }, nil
)
