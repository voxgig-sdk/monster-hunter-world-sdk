...
module MonsterHunterWorldUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = MonsterHunterWorldResponse.new({})
    ...
    if fetch_err
      ...
      response = MonsterHunterWorldResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MonsterHunterWorldResponse.new(fetched)
    else
