...
// MonsterHunterWorld SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class MonsterHunterWorldMakePoint
{
    public static function call(MonsterHunterWorldContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof MonsterHunterWorldError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = MonsterHunterWorldHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = MonsterHunterWorldHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
