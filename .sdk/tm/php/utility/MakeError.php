...
class MonsterHunterWorldMakeError
{
    public static function call(?MonsterHunterWorldContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new MonsterHunterWorldContext([], null);
        }
        $op = $ctx->op ?? new MonsterHunterWorldOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new MonsterHunterWorldResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof MonsterHunterWorldError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new MonsterHunterWorldError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof MonsterHunterWorldError) {
