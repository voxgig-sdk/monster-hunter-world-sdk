...
// MonsterHunterWorld SDK feature test
//
...
class FtCtrl extends MonsterHunterWorldControl
{
...
class FtClient
{
    ...
    private MonsterHunterWorldUtility $utility;

    public function __construct(MonsterHunterWorldUtility $utility, array $options)
    {
    ...
    public function get_utility(): MonsterHunterWorldUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MonsterHunterWorldContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MonsterHunterWorldContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MonsterHunterWorldContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MonsterHunterWorldConfig::make_feature($name);
    ...
    private static function buildUrl(MonsterHunterWorldSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MonsterHunterWorldError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MonsterHunterWorldResponse($fetched);
        }
    ...
    private function fail(MonsterHunterWorldContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MonsterHunterWorldContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
