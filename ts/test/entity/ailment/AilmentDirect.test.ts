...
describe('AilmentDirect', async () => {
  ...
  test('direct-load-ailment', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-ailment', setup.live)) return
  ...
  test('direct-list-ailment', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-ailment', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MONSTER_HUNTER_WORLD_TEST_AILMENT_ENTID']
