...
describe('MotionValueDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new MonsterHunterWorldSDK({
  ...
  test('direct-load-motion_value', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-motion_value', setup.live)) return
  ...
  test('direct-list-motion_value', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-motion_value', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MONSTER_HUNTER_WORLD_TEST_MOTION_VALUE_ENTID']
