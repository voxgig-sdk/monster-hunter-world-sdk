...
describe('WeaponDirect', async () => {
  ...
  test('direct-load-weapon', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-weapon', setup.live)) return
  ...
  test('direct-list-weapon', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-weapon', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MONSTER_HUNTER_WORLD_TEST_WEAPON_ENTID']
