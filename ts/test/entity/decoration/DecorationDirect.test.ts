...
describe('DecorationDirect', async () => {
  ...
  test('direct-load-decoration', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-decoration', setup.live)) return
  ...
  test('direct-list-decoration', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-decoration', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MONSTER_HUNTER_WORLD_TEST_DECORATION_ENTID']
