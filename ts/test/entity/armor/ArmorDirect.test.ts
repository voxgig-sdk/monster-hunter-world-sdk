...
describe('ArmorDirect', async () => {
  ...
  test('direct-load-armor', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-armor', setup.live)) return
  ...
  test('direct-list-armor', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-armor', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MONSTER_HUNTER_WORLD_TEST_ARMOR_ENTID']
