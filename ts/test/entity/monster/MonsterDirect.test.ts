...
describe('MonsterDirect', async () => {
  ...
  test('direct-load-monster', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-monster', setup.live)) return
  ...
  test('direct-list-monster', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-monster', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['MONSTER_HUNTER_WORLD_TEST_MONSTER_ENTID']
