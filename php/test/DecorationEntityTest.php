...
class DecorationEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Decoration(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Decoration(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MONSTER_HUNTER_WORLD_TEST_DECORATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $decoration_ref01_data_dt0_load_result = Helpers::to_map(is_object($decoration_ref01_data_dt0_loaded) && method_exists($decoration_ref01_data_dt0_loaded, 'data_get') ? $decoration_ref01_data_dt0_loaded->data_get() : $decoration_ref01_data_dt0_loaded);
...
function decoration_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("MONSTER_HUNTER_WORLD_TEST_DECORATION_ENTID");
