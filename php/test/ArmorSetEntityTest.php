...
class ArmorSetEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = MonsterHunterWorldSDK::test(null, null);
        $ent = $testsdk->ArmorSet(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->ArmorSet(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->ArmorSet(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = armor_set_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "armor_set." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $armor_set_ref01_data = null;
        if (count($armor_set_ref01_data_raw) > 0) {
...
function armor_set_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/armor_set/ArmorSetTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["armor_set01", "armor_set02", "armor_set03"] as $k) {
