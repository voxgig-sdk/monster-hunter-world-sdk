...
// MonsterHunterWorld SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        MonsterHunterWorldSDK $client,
        MonsterHunterWorldUtility $utility
    ): MonsterHunterWorldContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new MonsterHunterWorldContext($ctxmap, null);
    ...
    private static function fixctx(MonsterHunterWorldContext $ctx, MonsterHunterWorldSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?MonsterHunterWorldError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new MonsterHunterWorldError($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new MonsterHunterWorldResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new MonsterHunterWorldBaseFeature();
    ...
    {
        $hook_client = MonsterHunterWorldSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends MonsterHunterWorldBaseFeature {
            public $hook_fn = null;
            public function TestHook(MonsterHunterWorldContext $ctx): void
            {
    ...
    {
        $init_client = MonsterHunterWorldSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends MonsterHunterWorldBaseFeature {
            public $init_fn = null;
    ...
    {
        $calls = [];
        $live_client = new MonsterHunterWorldSDK([
            'system' => [
    ...
    {
        $blocked_client = new MonsterHunterWorldSDK([
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new MonsterHunterWorldSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = MonsterHunterWorldSDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new MonsterHunterWorldOperation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = MonsterHunterWorldSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof MonsterHunterWorldError) ? $result->err->msg : (string)$result->err;
