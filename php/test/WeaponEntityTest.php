...
class WeaponEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = MonsterHunterWorldSDK::test(null, null);
        $ent = $testsdk->Weapon(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Weapon(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Weapon(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = weapon_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "weapon." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MONSTER_HUNTER_WORLD_TEST_WEAPON_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $weapon_ref01_data = null;
        if (count($weapon_ref01_data_raw) > 0) {
...
function weapon_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/weapon/WeaponTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["weapon01", "weapon02", "weapon03"] as $k) {
    ...
    $entid_env_raw = getenv("MONSTER_HUNTER_WORLD_TEST_WEAPON_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["MONSTER_HUNTER_WORLD_TEST_WEAPON_ENTID"]);
