...
class MonsterHunterWorldSDK
{
        ...
        if ($feature_opts) {
            ...
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = MonsterHunterWorldHelpers::to_map($feature_opts[$fname] ?? null);
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new MonsterHunterWorldError($op . "_allow",
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new MonsterHunterWorldError("graphql_error",
                "MonsterHunterWorldSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
