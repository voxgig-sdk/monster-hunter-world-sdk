...
// MonsterHunterWorld SDK ArmorSet entity
...
class ArmorSetEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ArmorSetEntity($this->_client, $opts);
    }

    /**
     * @param ArmorSet|array $args ArmorSet data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = MonsterHunterWorldHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return ArmorSet|array The current ArmorSet data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of ArmorSet fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = MonsterHunterWorldHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of ArmorSet fields).
     */
    ...
    /**
     * Load a single ArmorSet.
     *
     * @param ArmorSetLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ArmorSetLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return ArmorSet|array The loaded ArmorSet as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = MonsterHunterWorldHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List ArmorSet items matching the given filter.
     *
     * @param ArmorSetListMatch|array|null $reqmatch Match filter (any subset
     *   of ArmorSet fields) as an assoc-array; ArmorSetListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return ArmorSet[]|array A list of ArmorSet items as assoc-arrays at
