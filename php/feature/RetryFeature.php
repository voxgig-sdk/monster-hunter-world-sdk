...
// MonsterHunterWorld SDK retry feature
...
class MonsterHunterWorldRetryFeature extends MonsterHunterWorldBaseFeature
{
    ...
    public function init(MonsterHunterWorldContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (MonsterHunterWorldContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(MonsterHunterWorldContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(MonsterHunterWorldContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
