...
[MCP](https://modelcontextprotocol.io) server exposing the MonsterHunterWorld SDK as
two agent tools — `monster-hunter-world_list` and `monster-hunter-world_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/monster-hunter-world-mcp)
make build
...
claude mcp add --scope user monster-hunter-world \
  -- /absolute/path/to/monster-hunter-world-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./monster-hunter-world-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// monster-hunter-world_list: first page of records
{ "entity": "ailment" }
{ "entity": "ailment", "query": { } }

// monster-hunter-world_load: one record by id
{ "entity": "ailment", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/monster-hunter-world-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user monster-hunter-world \
     -- "$PWD"/dist/*/monster-hunter-world-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `monster-hunter-world_list` and `monster-hunter-world_load` tools now appear
   in new sessions. Ask the agent to *"list ailment using monster-hunter-world"*
   and it calls `monster-hunter-world_list` with `{"entity":"ailment"}`.
...
### Call the `monster-hunter-world_list` tool
...
### Call the `monster-hunter-world_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `monster-hunter-world_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `monster-hunter-world_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Field | Type | Notes |
|-------|------|-------|
| `entity` | string | One of the 12 supported entities (see below). |
...
The 12 entities valid as the `entity` argument:
...
```sh
./monster-hunter-world-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"monster-hunter-world_load","arguments":{"entity":"ailment","query":{"id":1}}}}'
```
