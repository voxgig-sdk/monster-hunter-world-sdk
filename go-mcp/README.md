...
MCP server exposing the MonsterHunterWorld SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `monster-hunter-world_list` | `entity`, optional `query` map | First page of records as JSON |
| `monster-hunter-world_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
```sh
go build -o monster-hunter-world-mcp ./...
```
...
After install, restart Claude Code; the `monster-hunter-world_list` and
`monster-hunter-world_load` tools become available in new sessions.
