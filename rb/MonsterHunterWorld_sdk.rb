...
class MonsterHunterWorldSDK
    ...
    if feature_opts
      ...
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = MonsterHunterWorldHelpers.to_map(feature_opts[fname])
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => MonsterHunterWorldError.new(
        "#{op}_allow",
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = MonsterHunterWorldError.new(
        "graphql_error", "MonsterHunterWorldSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
