...
class ArmorSetEntity
  ...
  def make
    opts = @_entopts.dup
    ArmorSetEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = MonsterHunterWorldHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [ArmorSet, Hash] the current ArmorSet data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = MonsterHunterWorldHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of ArmorSet fields)
  def match_get
  ...
  #
  # @param reqmatch [ArmorSetLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.ArmorSet.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [ArmorSet, Hash] the loaded ArmorSet; raises MonsterHunterWorldError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = MonsterHunterWorldHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [ArmorSetListMatch, Hash, nil] match filter (any subset of
  #   ArmorSet fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<ArmorSet>, Array] the matching ArmorSet items; raises MonsterHunterWorldError on failure
