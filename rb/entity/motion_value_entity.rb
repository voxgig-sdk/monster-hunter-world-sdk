...
class MotionValueEntity
  ...
  def make
    opts = @_entopts.dup
    MotionValueEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.MotionValue.load works with no args).
  ...
  #
  # @param reqmatch [MotionValueListMatch, Hash, nil] match filter (any subset of
  #   MotionValue fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
