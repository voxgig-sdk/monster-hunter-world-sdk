...
class CharmEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MONSTER_HUNTER_WORLD_TEST_CHARM_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    charm_ref01_data_dt0_load_result = Helpers.to_map(charm_ref01_data_dt0_loaded.respond_to?(:data_get) ? charm_ref01_data_dt0_loaded.data_get : charm_ref01_data_dt0_loaded)
...
def charm_basic_setup(extra)
  ...
  entid_env_raw = ENV["MONSTER_HUNTER_WORLD_TEST_CHARM_ENTID"]
