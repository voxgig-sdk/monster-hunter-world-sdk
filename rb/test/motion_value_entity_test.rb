...
class MotionValueEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.MotionValue(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.MotionValue(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = motion_value_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "motion_value." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MONSTER_HUNTER_WORLD_TEST_MOTION_VALUE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    motion_value_ref01_data = nil
    if motion_value_ref01_data_raw.length > 0
    ...
    motion_value_ref01_data_dt0_load_result = Helpers.to_map(motion_value_ref01_data_dt0_loaded.respond_to?(:data_get) ? motion_value_ref01_data_dt0_loaded.data_get : motion_value_ref01_data_dt0_loaded)
...
def motion_value_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "motion_value", "MotionValueTestData.json")
  ...
  entid_env_raw = ENV["MONSTER_HUNTER_WORLD_TEST_MOTION_VALUE_ENTID"]
