...
class AilmentEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Ailment(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Ailment(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = ailment_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "ailment." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    ailment_ref01_data = nil
    if ailment_ref01_data_raw.length > 0
...
def ailment_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "ailment", "AilmentTestData.json")
