...
func TestArmorEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Armor(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Armor(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		armorRef01DataDt0LoadResult := core.ToMapAny(entityData(armorRef01DataDt0Loaded))
...
func armorBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MONSTER_HUNTER_WORLD_TEST_ARMOR_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MONSTER_HUNTER_WORLD_TEST_ARMOR_ENTID"])
