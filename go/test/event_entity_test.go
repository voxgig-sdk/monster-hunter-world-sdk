...
func TestEventEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MONSTERHUNTERWORLD_TEST_EVENT_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func eventBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MONSTERHUNTERWORLD_TEST_EVENT_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"MONSTERHUNTERWORLD_TEST_EVENT_ENTID": idmap,
		"MONSTERHUNTERWORLD_TEST_LIVE":      "FALSE",
		"MONSTERHUNTERWORLD_TEST_EXPLAIN":   "FALSE",
		"MONSTERHUNTERWORLD_APIKEY":         "NONE",
	})

	idmapResolved := core.ToMapAny(env["MONSTERHUNTERWORLD_TEST_EVENT_ENTID"])
