...
func TestArmorSetEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.ArmorSet(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.ArmorSet(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		armorSetRef01DataDt0LoadResult := core.ToMapAny(entityData(armorSetRef01DataDt0Loaded))
...
func armor_setBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MONSTER_HUNTER_WORLD_TEST_ARMOR_SET_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MONSTER_HUNTER_WORLD_TEST_ARMOR_SET_ENTID"])
