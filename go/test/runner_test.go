...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("MONSTERHUNTERWORLD_TEST_LIVE") == "TRUE" ||
		os.Getenv("MONSTERHUNTERWORLD_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("MONSTERHUNTERWORLD_TEST_EXPLAIN"); explain != "" {
